...
- **`SKILL.md` — Step 14 expanded with a "Bootstrap the wiki" subsection.** Every generated project now ships with a starter GitHub wiki: 12 pages seeded by the skill (Home, _Sidebar, _Footer, Getting-Started, How-It-Works, Hard-Rules, Six-Tier-Safety-Classifier, Build-Process, ADR-Index, FAQ, Roadmap, Contributing-to-the-Wiki), cloned locally at `wiki/` (gitignored from the main repo so the nested `.git/` doesn't poison the parent's `git status`), and pushed to `<repo>.wiki.git`. The skill also instructs the AI to add a "Contributing to the wiki" section to the generated `CONTRIBUTING.md` documenting the clone-edit-push workflow and the `docs/` vs. wiki split. The subsection includes three lessons learned the hard way: (1) `gh repo create <slug>.wiki` fails — GitHub reserves the `.wiki` suffix for auto-provisioned wiki repos, the only path is via the web UI; (2) `<slug>.wiki.git` does not exist until the operator saves a first page through GitHub's Wiki tab — the AI must STOP and ask the operator to do this manually before the clone will succeed; (3) GitHub-provisioned wiki repos default to the `master` branch, not `main`, so commits and pushes use `master`. Rationale: GitHub wikis are a low-friction surface for external contributors; codifying the seed set means every project this skill builds has the same human-readable framing on day one, not just for repos where someone happens to remember.
...
- **GitHub Wiki adopted as a documentation surface for this repo.** 12 starter pages authored under `wiki/` (a separate git repository pointed at `<repo>.wiki.git`, gitignored from the main repo): Home, _Sidebar, _Footer, Getting-Started, How-It-Works, Hard-Rules, Six-Tier-Safety-Classifier, Build-Process, ADR-Index, FAQ, Roadmap, Contributing-to-the-Wiki. Pages link to canonical content in `docs/` and `SKILL.md` via absolute github.com URLs; `docs/` remains the tie-breaker when the two disagree. The wiki is a human-readable companion, not a replacement.
- **`.gitignore` — added `wiki/`** with an inline comment explaining the nested-repo pattern. Without the ignore, the main repo would treat `wiki/.git` as an accidental gitlink and break `git status` / `git add`.
- **`CONTRIBUTING.md` — new "Contributing to the wiki" section.** Documents the clone-edit-push workflow, the `Page-Name.md` / `[[Page Name]]` / absolute-link conventions, the `docs/` vs. wiki content split (canonical vs. human-readable framing), and the rule that the safety contract / tokens / personal info never go on the wiki.
